...
var serviceEnableSSLCmd = &cobra.Command{
  ...
  To enable ssl for the app on gcp:
...
var serviceSetStaticIpCmd = &cobra.Command{
	Use:   "set-static-ip",
	Short: "Set static IP for the app, if using an ingress (GCP only)",
	Long: `Set static IP for the app, if using an ingress (GCP only).

  To set a static IP on an app:
